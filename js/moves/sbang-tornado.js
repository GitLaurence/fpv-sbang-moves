export default {
  id: 'sbang-tornado',
  name: 'Sbang Tornado',
  level: 'pro',
  difficulty: 5,
  durationSec: 6.5,
  youtubeId:    'BQauW42gMfE',
  youtubeStart: 0,
  description: 'Sbang-style boost → roll inverted → dead stop → multiple inverted yaw spins ("tornado") → Split-S pull-through → Juicy Flick → roll out. Pinakamahaba at pinakamabigat na sbang variant — kailangan ng matatag na inverted hover bago ang spin.',
  tips: [
    'Manatiling dead stop muna bago simulan ang spin — kung gumagalaw pa ang quad, mawawalan ng control ang spin',
    'I-maintain ang elevated throttle (~65-70%) habang umiikot inverted para hindi bumagsak',
    'I-stop ang yaw nang eksakto bago ang Split-S pull — kung hindi, magiging skewed ang exit',
  ],
  phases: [
    { t: 0.0, label: 'Entry' },
    { t: 0.4, label: 'Boost' },
    { t: 0.8, label: 'Roll Inv' },
    { t: 1.2, label: 'Dead Stop' },
    { t: 1.5, label: 'Tornado' },
    { t: 4.2, label: 'Stop Spin' },
    { t: 4.5, label: 'Pull S' },
    { t: 5.4, label: 'Flick' },
    { t: 5.9, label: 'Roll Out' },
  ],
  keyframes: [
    // Entry — level forward flight
    { t: 0.0,  throttle: 0.68, yaw:  0.00, pitch:  0.00, roll:  0.00 },
    { t: 0.25, throttle: 0.68, yaw:  0.00, pitch:  0.00, roll:  0.00 },
    // BOOST — full throttle punch with slight nose-up
    { t: 0.4,  throttle: 1.00, yaw:  0.00, pitch: -0.25, roll:  0.00 },
    { t: 0.65, throttle: 1.00, yaw:  0.00, pitch: -0.15, roll:  0.00 },
    // Roll to inverted — snap full roll, chop throttle
    { t: 0.8,  throttle: 0.35, yaw:  0.00, pitch:  0.00, roll:  1.00 },
    { t: 1.0,  throttle: 0.30, yaw:  0.00, pitch:  0.00, roll:  1.00 },
    // DEAD STOP — fully inverted, all sticks center, brief pause
    { t: 1.2,  throttle: 0.28, yaw:  0.00, pitch:  0.00, roll:  0.00 },
    { t: 1.4,  throttle: 0.28, yaw:  0.00, pitch:  0.00, roll:  0.00 },
    // TORNADO — sustained inverted yaw spin, elevated throttle to hold altitude
    { t: 1.5,  throttle: 0.68, yaw:  1.00, pitch:  0.00, roll:  0.00 },
    { t: 2.0,  throttle: 0.70, yaw:  1.00, pitch:  0.00, roll:  0.00 },
    { t: 2.5,  throttle: 0.67, yaw:  1.00, pitch:  0.00, roll:  0.00 },
    { t: 3.0,  throttle: 0.70, yaw:  1.00, pitch:  0.00, roll:  0.00 },
    { t: 3.5,  throttle: 0.68, yaw:  1.00, pitch:  0.00, roll:  0.00 },
    { t: 4.0,  throttle: 0.70, yaw:  1.00, pitch:  0.00, roll:  0.00 },
    // Stop the spin — center yaw stick
    { t: 4.2,  throttle: 0.65, yaw:  0.00, pitch:  0.00, roll:  0.00 },
    { t: 4.35, throttle: 0.65, yaw:  0.00, pitch:  0.00, roll:  0.00 },
    // SPLIT-S pull-through — sustained pitch back arcs nose down
    { t: 4.5,  throttle: 0.35, yaw:  0.00, pitch: -1.00, roll:  0.00 },
    { t: 4.85, throttle: 0.52, yaw:  0.00, pitch: -0.95, roll:  0.00 },
    { t: 5.2,  throttle: 0.68, yaw:  0.00, pitch: -0.55, roll:  0.00 },
    // Recovery — level out, brief pause before the juicy flick
    { t: 5.4,  throttle: 0.70, yaw:  0.00, pitch:  0.00, roll:  0.00 },
    // JUICY FLICK — throttle blip then pitch snap to inverted
    { t: 5.55, throttle: 0.85, yaw:  0.00, pitch:  0.00, roll:  0.00 },
    { t: 5.7,  throttle: 0.28, yaw:  0.00, pitch:  1.00, roll:  0.00 },
    { t: 5.9,  throttle: 0.25, yaw:  0.00, pitch:  0.00, roll:  0.00 },
    // 180° roll exit back to upright
    { t: 6.05, throttle: 0.38, yaw:  0.00, pitch:  0.00, roll:  1.00 },
    { t: 6.25, throttle: 0.55, yaw:  0.00, pitch:  0.00, roll:  0.50 },
    { t: 6.4,  throttle: 0.65, yaw:  0.00, pitch:  0.00, roll:  0.00 },
    // Settle
    { t: 6.5,  throttle: 0.65, yaw:  0.00, pitch:  0.00, roll:  0.00 },
  ],
};
